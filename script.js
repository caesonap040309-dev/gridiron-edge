...
function sharpMetrics(game,p){
  ...
  return `<div class="sharp-metrics"><div><span>Spread edge</span><b>${p.spreadEdge==null?"—":signed(p.spreadEdge)+" pts"}</b><small>${coverProb==null?"Waiting for market":esc(coverSide)+" "+coverProb.toFixed(1)+"%"}</small></div><div><span>Total edge</span><b>${p.totalEdge==null?"—":signed(p.totalEdge)+" pts"}</b><small>${totalProb==null?"Waiting for market":totalSide+" "+totalProb.toFixed(1)+"%"}</small></div><div><span>Fair moneyline</span><b>${esc(game.home)} ${american(p.fairHomeMoneyline)}</b><small>${esc(game.away)} ${american(p.fairAwayMoneyline)}</small></div><div><span>Model confidence</span><b>${esc(p.confidence||"Low")}</b><small>${Number(p.sample)||0} games minimum · ${Number(p.marketBooks)||0} books</small></div></div>${p.adjustments?`<div class="model-factors"><span>Model v${Number(p.version)||"—"}</span><span>${p.adjustments.neutralSite?"Neutral site":"Home field "+signed(p.adjustments.homeField)}</span><span>Rest ${signed(p.adjustments.rest)}</span><span>Venue form ${signed(p.adjustments.venue)}</span><span>Weather total ${signed(p.adjustments.weatherTotal)}</span></div>`:""}`;
}
