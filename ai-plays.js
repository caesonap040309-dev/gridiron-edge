(()=>{
  ...
  async function load(){
    ...
    const plays=candidates.map(row=>{const value=Number(row.valueEdge),base=Number.isFinite(value)?value:Math.max(Number(row.edge)||0,Math.abs(Number(row.prob)-50));const units=stakeFor(row);return {...row,units,score:base*row.q.weight}}).sort((a,b)=>b.units-a.units||(confidenceRank[b.tier]||0)-(confidenceRank[a.tier]||0)||b.score-a.score||b.prob-a.prob||b.q.weight-a.q.weight).slice(0,10);
    const source=sourceLabel(),title=document.getElementById("aiTitle"),list=document.getElementById("aiPlays"),note=document.getElementById("aiNote");if(title)title.textContent=`${label} Week ${selectedWeek} Top 10 ${source} Plays`;if(note)note.textContent=selectedSource==="props"?"Ranked from a 50/50 blend of independent recent-game statistics and no-vig sportsbook consensus. Props without enough game-log data are excluded.":selectedSource==="games"?"Ranked by confidence first, then value versus the sportsbook price. Standard-priced spreads and totals are prioritized; expensive moneylines are excluded.":"One Top 10 board combining the strongest eligible plays. Unit ratings are edge-gated; weak or noisy spots are marked NO BET instead of being forced into a wager.";if(!list)return;
