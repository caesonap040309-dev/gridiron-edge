...
function auditLeague(data,league){
  ...
  for(const game of data?.games||[]){
    ...
    if(homeScore!==awayScore&&Number.isFinite(Number(p.homeWin))){const homePick=Number(p.homeWin)>=50,outcome=homeScore>awayScore;rows.push({league,market:"moneyline",confidence:p.confidence||"Unknown",prob:(homePick?Number(p.homeWin):100-Number(p.homeWin))/100,outcome:homePick===outcome?1:0,result:homePick===outcome?"win":"loss"})}
    const point=Number(p.market?.homePoint),homeCover=Number(p.homeCover),coverMargin=homeScore-awayScore+point;
    if(Number.isFinite(point)&&Number.isFinite(homeCover)){const homePick=homeCover>=50;rows.push({league,market:"spread",confidence:p.confidence||"Unknown",prob:(homePick?homeCover:100-homeCover)/100,outcome:coverMargin===0?.5:(homePick===(coverMargin>0)?1:0),result:coverMargin===0?"push":(homePick===(coverMargin>0)?"win":"loss")})}
    const line=Number(p.market?.total),overProb=Number(p.overProb),difference=homeScore+awayScore-line;
    if(Number.isFinite(line)&&Number.isFinite(overProb)){const overPick=overProb>=50;rows.push({league,market:"total",confidence:p.confidence||"Unknown",prob:(overPick?overProb:100-overProb)/100,outcome:difference===0?.5:(overPick===(difference>0)?1:0),result:difference===0?"push":(overPick===(difference>0)?"win":"loss")})}
  }
