...
try{dailyCalibration=JSON.parse(await readFile("data/model-calibration.json","utf8")).nfl||dailyCalibration}catch{}
...
async function fetchSportsGameOdds(){
  const key=(process.env.SPORTSGAMEODDS_API_KEY||process.env.SPORTS_GAME_ODDS_API_KEY)?.trim();
  if(!key)return [];
...
async function fetchHistoricalSeason(year){
  ...
  for(let week=1;week<=maxWeek;week++){
    const q=new URLSearchParams({limit:"100",dates:String(year),seasontype:"2",week:String(week)});
    const response=await fetch(`${espn}?${q}`);
...
for(const game of games){
  ...
  const confidence=sample>=6&&market.bookCount>=2&&marketStable&&maxEdge>=4&&reliability>=.82?"High":sample>=3&&market.bookCount>=1&&marketStable&&maxEdge>=2?"Medium":"Low";
  const prediction={
